...
final userRouterProvider = Provider<GoRouter>((ref) {
  return GoRouter(
    initialLocation: UserRoutes.splash,
    ...
    redirect: (context, state) {
      ...
      // Đã đăng nhập → không cần vào trang login nữa
      if (user != null && loc.startsWith('/login')) {
        return '/generate';
      }
      // Landing page (/) luôn accessible — kể cả khi đã đăng nhập
      return null;
    },
    routes: [
      // ── Splash — chỉ hiện lúc khởi động, sau đó điều hướng dựa theo auth ──
      GoRoute(
        ...
        builder: (context, state) {
          ...
          return SplashScreen(
            ...
            onFinish: (isAuthenticated) {
              // Đã đăng nhập → vào thẳng /generate, bỏ qua landing page
              // Chưa đăng nhập → hiển thị landing page
              context.go(
                isAuthenticated ? UserRoutes.generate : UserRoutes.landing,
              );
            },
          );
        },
